...
No. This states that there exists an X that loves every Y. This is not the case.

2. ∃y. ∀x. loves(x, y)

Yes. There exists a Y that is loved by every X. This is true for Y=c.

3. ∀x. ∃y. loves(x, y)

Yes. Every X loves at least one Y.

4. ∀y. ∃x. loves(x, y)

Yes. For every y, there is one X that loves them.

5. ∃x. ∃y. loves(x, y)

Yes. There exists an X that loves a Y.

6. ∃x. ∀y. ¬loves(x, y)

No. There does not exist an X that does not love every Y.

7. ∃x. ¬∀y. loves(x, y)

No. There does not exist an X that loves no Ys.
...
I'm a little confused because this isn't a recursive function? You could 
probably write it so that it is recursive, although this seems unneccessary.

b. Is `max_subarray` an example of divide and conquer? Why or why not?

No because it is an iterative solution. You can solve it by iterating through 
the array once.

c. Is `max_subarray` an example of dynamic programming? Why or why not?

No. There's no "subproblem" to memoize.
...
Still confused because of the above.
...
bs_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 9) ->
bs_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 4) ->
bs_array_helper(3, [1, 3, 4, 6, 7, 8, 10, 13, 14], 0, 2) -> return 1


b. Is `binary_search_array` an example of divide and conquer? Why or why not?

No, because you never have to solve two or more subproblems at the same time. 
You do divide the problem, but you only solve one of the subproblems.

c. Is `binary_search_array` an example of dynamic programming? Why or why not?

Nope. Again, you don't have to memoize a solution and reuse it.
...
I don't entirely understand why you would memoize this algorithm since you can 
perform a binary search in logn time without every having to solve the same 
problem twice. Maybe I'm just misunderstanding these questions? I will ask in 
class.
